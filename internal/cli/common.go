...
func bindAuthFlags(cmd *cobra.Command, flags *commonFlags) {
	...
	cmd.Flags().StringVar(&flags.gitSSHKeyFile, "git-ssh-key-file", flags.gitSSHKeyFile, "private key file for Git SSH sources (default: ssh-agent, then ~/.ssh/config IdentityFile and ~/.ssh/id_*)")
	...
	cmd.Flags().StringVar(&flags.gitKnownHostsFile, "git-known-hosts-file", flags.gitKnownHostsFile, "known_hosts file for Git SSH sources (default: $SSH_KNOWN_HOSTS, then ~/.ssh/known_hosts)")
	...
	cmd.Flags().StringVar(&flags.ociCAFile, "oci-ca-file", flags.ociCAFile, "PEM CA bundle for OCI artifact registry and OCI Helm chart registry TLS (artifact registries: replaces the system pool; Helm chart registries: added to the system pool)")
	cmd.Flags().StringVar(&flags.ociClientCertFile, "oci-client-cert-file", flags.ociClientCertFile, "client certificate file for mutual-TLS OCI artifact registries and OCI Helm chart registries (requires --oci-client-key-file)")
	cmd.Flags().StringVar(&flags.ociClientKeyFile, "oci-client-key-file", flags.ociClientKeyFile, "client key file for mutual-TLS OCI artifact registries and OCI Helm chart registries (requires --oci-client-cert-file)")
	cmd.Flags().BoolVar(&flags.ociInsecureSkipVerify, "oci-insecure-skip-verify", flags.ociInsecureSkipVerify, "skip TLS certificate verification for OCI artifact registries and OCI Helm chart registries (insecure; credentials are exposed to whoever answers)")
